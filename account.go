...
func (tmdb *TMDb) GetAccountRatedTv(id int, sessionID string, options map[string]string) (*TvRatingResults, error) {
	var availableOptions = map[string]struct{}{
		"page":     {},
		"sort_by":  {},
		"language": {}}
	var favorites TvRatingResults
	optionsString := getOptionsString(options, availableOptions)
	uri := fmt.Sprintf("%s/account/%v/rated/tv?api_key=%s&session_id=%s%s", baseURL, id, tmdb.apiKey, sessionID, optionsString)
	result, err := getTmdb(uri, &favorites)
	return result.(*TvRatingResults), err
}

// GetAccountRatedTvEpisodes gets the list of rated TV episodes (and associated rating) for an account
// https://developers.themoviedb.org/3/account/get-rated-tv-episodes
func (tmdb *TMDb) GetAccountRatedTvEpisodes(id int, sessionID string, options map[string]string) (*TvEpisodeRatingResults, error) {
	var availableOptions = map[string]struct{}{
		"page":     {},
		"sort_by":  {},
		"language": {}}
	var favorites TvEpisodeRatingResults
	optionsString := getOptionsString(options, availableOptions)
	uri := fmt.Sprintf("%s/account/%v/rated/tv/episodes?api_key=%s&session_id=%s%s", baseURL, id, tmdb.apiKey, sessionID, optionsString)
	result, err := getTmdb(uri, &favorites)
	return result.(*TvEpisodeRatingResults), err
}
